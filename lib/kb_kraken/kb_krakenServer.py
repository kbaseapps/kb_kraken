...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'kb_kraken'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from kb_kraken.kb_krakenImpl import kb_kraken
impl_kb_kraken = kb_kraken(config)
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'kb_kraken'
        ...
        self.rpc_service.add(impl_kb_kraken.run_kraken,
                             name='kb_kraken.run_kraken',
                             types=[dict])
        self.method_authentication['kb_kraken.run_kraken'] = 'required'
        self.rpc_service.add(impl_kb_kraken.status,
                             name='kb_kraken.status',
                             types=[dict])
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "kb_kraken but no authentication header was passed"
                            raise err
